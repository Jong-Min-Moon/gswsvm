...
draw.basic <- function(dataset, col.p, col.n, alpha.p, alpha.n){
  ggplot.object <- ggplot() +
    geom_point(data = data.train[data.train$y == "neg", ], aes(x = x1, y = x2), shape = 1, col = col.n, alpha = alpha.n) +
    geom_point(data = data.train[data.train$y == "pos", ], aes(x = x1, y = x2), shape = 16, col = col.p, alpha = alpha.p)
