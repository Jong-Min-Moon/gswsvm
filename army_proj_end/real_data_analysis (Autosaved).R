...
national.neg <- national[(national$DMG) == 0, ]
national.neg.predictors <- national.neg[predictors]
national.pos <- national[(national$DMG) == 1, ]
...
for (national.num in rownames(national.pos.predictors)){
	national.vector <- national.pos.predictors[national.num, ]
	for (army.num in rownames(army.predictors)){
		army.vector <- army.predictors[army.num, ]
		l2.distances.pos[national.num, army.num] <- sqrt(sum((army.vector - national.vector)^2))
	}
}

l2.distances.pos.mean <- apply(l2.distances.pos, 1, mean)
l2.distances.pos.mean <- sort(l2.distances.pos.mean)
new.positive.distances <- l2.distances.pos.mean[1:30]
...
#Peter Willett
start <- army[ ]
samplePool <- national[ predictors ]
