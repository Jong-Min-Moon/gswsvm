...
source("simplifier.r", print.eval  = TRUE, echo = TRUE, verbose=TRUE)
...
imbalance.ratio.original <- 1032/40
#imbalance.ratios.desired <- c(imbaland.ratio.original, 20, 15, 10, 5)
imbalance.ratios.desired <- c( 20, 15, 10, 5)
...
imbal.gme <- matrix(0, nrow = length(imbalance.ratios.desired), ncol = n.method)
rownames(imbal.gme) <- imbalance.ratios.desired
...
for (imbalance.ratio.desired in imbalance.ratios.desired){
  oversample.ratio <- imbaland.ratio.original / imbalance.ratio.desired  - 1
  for (rep in 1:replication){
    ...
    for (foldnum.test in 1:5){
      ...
      data.train <- data.full[indices.train, ] # 1 - test.ratio
      data.test  <- data.full[indices.test , ] # test.ratio
      ...
      for (foldnum.tune in 1:5){
        ...
        weight.svmdc<- imbalance.ratio.desired * (data.gswsvm.train$y == 'pos') + 1 * (data.gswsvm.train$y == 'neg')
...
} # end of for loop for imbalance ratios
